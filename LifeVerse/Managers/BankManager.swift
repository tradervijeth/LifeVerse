...
class BankManager: ObservableObject, Codable {
                    ...
                    let loanDueEvent = LifeEvent(
                        ...
                        choices: [
                            ...
                            EventChoice(
                                ...
                                effects: [EventChoice.CharacterEffect(attribute: "creditScore", change: -5)]
                            )
                        ]
                    )
                    events.append(loanDueEvent)
                }
            }
        }
        return events
    ...
    // Update market conditions based on random factors
    func updateMarketConditions() {
        // Random chance to change market conditions
        let changeChance = Double.random(in: 0...1)

        if changeChance < 0.3 { // 30% chance to change market conditions
            // Determine direction of change
            let currentIndex = [MarketCondition.depression, .recession, .recovery, .expansion, .boom].firstIndex(of: marketCondition) ?? 2

            // Tendency to move toward recovery (mean reversion)
            let moveUpProbability: Double
            switch marketCondition {
            case .depression: moveUpProbability = 0.8 // 80% chance to improve from depression
            case .recession: moveUpProbability = 0.7 // 70% chance to improve from recession
            case .recovery: moveUpProbability = 0.5 // 50/50 chance from recovery
            case .expansion: moveUpProbability = 0.3 // 30% chance to improve from expansion
            case .boom: moveUpProbability = 0.1 // 10% chance to improve from boom (unlikely)
            }

            // Determine if moving up or down
            let isMovingUp = Double.random(in: 0...1) < moveUpProbability

            // Calculate new index with bounds checking
            let newIndex: Int
            if isMovingUp {
                newIndex = min(4, currentIndex + 1) // Move up (better economy)
            } else {
                newIndex = max(0, currentIndex - 1) // Move down (worse economy)
            }

            // Set new market condition
            let allConditions = [MarketCondition.depression, .recession, .recovery, .expansion, .boom]
            marketCondition = allConditions[newIndex]
        }
    }

    // Save accounts to persistent storage
    func saveAccounts() {
        // This would typically save to UserDefaults, CoreData, or a file
        // For now, this is a placeholder for the actual implementation
        // The actual implementation would depend on the app's storage strategy
    ...
    // Get net worth (alias for calculateNetWorth for consistency)
    ...
    // Transfer money between accounts using account IDs
    func transferMoney(fromAccountId: UUID, toAccountId: UUID, amount: Double, completion: @escaping (Bool, String) -> Void) {
        // Check if both accounts exist
        guard let fromIndex = accounts.firstIndex(where: { $0.id == fromAccountId }),
              let toIndex = accounts.firstIndex(where: { $0.id == toAccountId }) else {
            completion(false, "One or both accounts do not exist")
            return
        }

        // Check if the from account has sufficient funds
        guard accounts[fromIndex].balance >= amount else {
            completion(false, "Insufficient funds in the account")
            return
        }
        ...
        let timestamp = Date()
        let currentYear = Calendar.current.component(.year, from: timestamp)
        ...
        // Add to global transaction history
        let fromTransaction = BankTransaction(
            type: .transfer,
            amount: -amount,
            description: "Transfer to \(accounts[toIndex].accountType.rawValue)",
            date: timestamp,
        ...
        let toTransaction = BankTransaction(
            type: .transfer,
            amount: amount,
            description: "Transfer from \(accounts[fromIndex].accountType.rawValue)",
            date: timestamp,
        ...
        transactionHistory.append(fromTransaction)
        transactionHistory.append(toTransaction)

        // Save updated accounts
        saveAccounts()

        completion(true, "Successfully transferred $\(amount) from account \(fromAccountId) to \(toAccountId)")
    }
